...
type TestResourceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.TestResource, err error)
...
// List lists all TestResources in the indexer.
func (s *testResourceLister) List(selector labels.Selector) (ret []*v1alpha1.TestResource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.TestResource))
	})
	return ret, err
}
...
type TestResourceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.TestResource, error)
...
// List lists all TestResources in the indexer for a given namespace.
func (s testResourceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.TestResource, err error) {
...
// Get retrieves the TestResource from the indexer for a given namespace and name.
func (s testResourceNamespaceLister) Get(name string) (*v1alpha1.TestResource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("testresource"), name)
	}
	return obj.(*v1alpha1.TestResource), nil
}
